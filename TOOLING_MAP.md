...
| Tool | Status | Purpose |
|---|---|---|
| **Playwright headless suite** | **Built & primary safety net** — 14 suites + 115-check regression trace, all green, in version control (`tests/`) | Drives the REAL game (bootGame harness, real state transitions per KNOWLEDGE §2b): gate engine, NPC/memory log, widgets/inventory, export purity, calendar, game log, loadout + shop widget screens, occlusion/§11 viewport gates, save fuzz, visual probe (B2: 9 structural screenshots + HUD pixel regions + blank-detector control + loadout overflow pin), perf budget (B10: per-tick CPU budgets, idle + 120-enemy stress, overload negative control), plus the full historical trace. `npm test` (skip-safe) / `npm run test:strict` (skips fail) / `npm run test:trace`. Artifacts in `tests/artifacts/` |
