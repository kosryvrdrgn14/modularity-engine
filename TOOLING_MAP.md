...
| Widget Inspector | **Built (v2.19.12)** (`widget_ui_system_spec.md` §6.4) | `window.__WIDGET_DEBUG__` console bridge over `WidgetRenderer._all`: `list()` every live card + its producing def summary and data across all renderer instances; `inspect(el)` full def JSON + resolved data + payload preview + geometry + live `elementFromPoint` clickable check; `occlusion()` the §7.2 audit across every live interactive instance (same detector the occlusion suite gates on, now available live in any game state) | Dev-only console bridge — no UI, no ids; pinned in step3 incl. the veiled negative control |
...
| Tool | Status | Purpose |
|---|---|---|
| **Playwright headless suite** | **Built & primary safety net** — 15 suites + 115-check regression trace, all green, in version control (`tests/`) | Drives the REAL game (bootGame harness, real state transitions per KNOWLEDGE §2b): gate engine, NPC/memory log, widgets/inventory, export purity, calendar, game log, loadout + shop widget screens, occlusion/§11 viewport gates, save fuzz, visual probe (B2: 9 structural screenshots + HUD pixel regions + blank-detector control + loadout overflow pin), perf budget (B10: per-tick CPU budgets, idle + 120-enemy stress, overload negative control), §12 UI layout audit (gaps/dead-bands/alignment/contrast, desktop gates), plus the full historical trace. `npm test` (skip-safe) / `npm run test:strict` (skips fail) / `npm run test:trace`. Artifacts in `tests/artifacts/` |
...
| **POT-006 content pipeline** | Built & documented-here (17 content files since v2.19.7) | `public/content/*.json` are the single content source. Registration contract: a new content file joins BOTH `DataManager.loadAll()`'s fetch list (`engine/core.js`) AND the generator registry (`tools/generateEmbeddedData.mjs`) — miss either and the mirror goes out of sync / the orphan guard errors. Scripts: `npm run content:sync` (regenerate `public/data/embeddedData.js`), `npm run content:check` (byte-verify). Build-time condition validation runs through the shared ConditionEngine |
...
| Occlusion detection (`elementFromPoint`) | **Built (v2.12.0; §11 viewport matrix v2.12.1; clipped category v2.19.16)** (`widget_ui_system_spec.md` §7+§11) | Every interactive widget instance must actually be clickable at its position AND fully within the viewport, across a 3-viewport matrix (desktop 1280×800 GATING; mobile portrait/landscape REPORT-ONLY until promoted per §11); four-state taxonomy (unpresented/clipped/occluded/contextBuried); non-vacuous (burying-overlay + off-screen-card negative controls); same clipped report in `__WIDGET_DEBUG__.occlusion()`; runs in the battery + `npm run widget:audit` | `tests/suites/widget_occlusion.cjs` |
