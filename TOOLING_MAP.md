...
| Tool | Status | Purpose |
|---|---|---|
| **Playwright headless suite** | **Built & primary safety net** — 13 suites + 115-check regression trace, all green, in version control (`tests/`) | Drives the REAL game (bootGame harness, real state transitions per KNOWLEDGE §2b): gate engine, NPC/memory log, widgets/inventory, export purity, calendar, game log, loadout + shop widget screens, occlusion/§11 viewport gates, save fuzz, visual probe (B2: 8 structural screenshots + HUD pixel regions + blank-detector control), plus the full historical trace. `npm test` (skip-safe) / `npm run test:strict` (skips fail) / `npm run test:trace`. Artifacts in `tests/artifacts/` |
