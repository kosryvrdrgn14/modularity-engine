...
- 🟩 `step6_shop_tabs.cjs` — shop chips/selected-bind, v1.3 disabled suppression, B12 purchase-confirm flow (tap opens panel with NO gold movement; stepper clamp [1,affordable] + live total; scrim-tap cancel negative control; qty commit = one transaction, stack count=qty; full-description reveal surface), tab round-trip (former pilot pool bug), def-swap skin evidence (18)
...
- 🟩 `ui_layout_audit.cjs` — §12 UI polish standards, mechanized (v2.19.18): gaps on the 4-pt scale, dead bands (coverage-interval model — a run is dead iff nothing paints it), section alignment ±2px, size-aware effective-bg contrast; 4 negative controls (off-scale gap 18px, 130px dead band, misaligned block, #555-on-dark); report-first across 4 structural screens × 3 viewports → all clean → desktop gates promoted, mobile report-only; whitespace ratio reported per screen; v2.19.20 added overflow (doc/panel scrollWidth; defect = user-experienced overflow: auto/scroll/visible — by-design overflow:hidden clips like the town carousel exempt) + half-spec-ellipsis wrap detection (report-only) with 3 more negative controls (29); v2.19.21 added REAL device-emulation gates (iPhone 13 profile via harness `newMobilePage`: isMobile+hasTouch+DPR3+mobile UA — the same 5 probes gated per screen on a true mobile context) + report-only §11 touch-target (<44px) inventory per screen; v2.19.22 orientation-flip stability gates on the emulated page (portrait↔landscape, no user-experienced overflow in either); v2.19.24 wraps probe PROMOTED to a gate (desktop + emulated cells — B12 confirm panel fixed the shop wraps; the panel is the reveal surface, §12.8); v2.19.25 touch-target inventory PROMOTED to a gate on the emulated cells (desktop report-only — precision pointers allow dense targets) with a 20×20 negative control, after the pointer:coarse CSS block fixed all 22 enumerated sub-44px targets; v2.19.27 slotpicker added to the screen matrix (user-reported mobile clipping → auto-fit grid fix; §12.2 gained center-frame alignment — edges or center axis — with negative control); v2.19.28 levelup added to the matrix (same B15 class) + coarse-pointer portrait stacking for both card pickers (95)
