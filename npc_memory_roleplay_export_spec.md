...
```json
{
  ...
  "timestamp": "in-game-time or real-time, TBD — pick one and be consistent"
}
```
...
- `spoilerTag` is optional but strongly recommended on any event that reveals significant lore —
  this is what the gate engine keys off of when deciding what a given checkpoint's export may
  safely include (§6).
...
A checkpoint is a **named, curated** point in an NPC's (or NPC group's) history — not every event
becomes a checkpoint, or the list becomes unusable noise.

### 3.1 Checkpoint record shape

```json
{
  "checkpointId": "elena_ch2_trial_complete",
  "npcIds": ["elena"],
...
- `eventCutoff` marks "replay the log up through this event" — this is how a snapshot at this
  checkpoint gets reconstructed.
- `autoGenerated: false` should be reserved for a possible future "manual checkpoint" feature
...
### 3.2 What qualifies as checkpoint-worthy (curation policy)

Auto-generate a checkpoint on: affection tier changes, quest chain completions (especially the
...
A favorite is a **(checkpoint-set, NPC-set) pair**, not a single NPC — this is what enables both
...
```json
{
  ...
  "checkpoints": [
    { "npcId": "elena", "checkpointId": "elena_ch2_trial_complete" },
    { "npcId": "spider_queen", "checkpointId": "spiderqueen_claim_tier" }
  ],
...
Note each NPC in a favorite can reference a *different* checkpoint for herself — a group favorite
...
- **Facts (dynamic, derived per checkpoint):** everything reconstructed by replaying the event log
  up to the checkpoint's cutoff, filtered through the spoiler gate (§7).

This split keeps generation cheap — flavor never needs recomputation; only the facts section is
re-derived per checkpoint/character combination.
...
availability) as a third consumer. Query shape: *"given this checkpoint's event cutoff, which
...
- Spoiler containment — never reference or act on facts not yet revealed as of this checkpoint.
  *Generated directly from the gate evaluator's output for this checkpoint — not hand-written.*
- Relationship-status accuracy — behavior matches the actual affection tier/marital status at this
  checkpoint, not any later or earlier state.
...
1. A fast Tier 1 romance with minimal checkpoint history.
2. A hard-route Tier 3 character, at a checkpoint *before* her arc resolves.
3. A three-character group favorite spanning different affection tiers with each other and the
   player.
4. An early save with zero favorited checkpoints (menu/empty-state check, not a conversation
   test).
...
| 2 | **Spoiler leakage** | Ask her directly, then via leading questions, about facts she shouldn't know yet at this checkpoint. Does she hold the line? |
| 3 | **Relationship-status accuracy** | Ask her to describe her relationship to the player. Does it match the checkpoint's actual tier — not over- or under-stated? |
...
(c) a sign the underlying data model (facts/flavor split, checkpoint granularity) needs a
...
- Exact `timestamp` convention for events (in-game calendar time vs. real time) — needs a decision
  before implementation, doesn't block the no-code testing plan above.
- Whether manual (player-triggered) checkpoints are worth adding in v1 or deferred — the schema
  already reserves room for it (§3.1) either way.
