class Renderer {
  ...
  _updateAndDrawCones(dt) {
    ...
    for (let i = this.coneEffects.length - 1; i >= 0; i--) {
      ...
      c.age += dt || 1/60;
      if (c.age >= c.maxAge) { this.coneEffects.splice(i, 1); continue; }
      const progress = c.age / c.maxAge;
      const alpha = 0.4 * (1 - progress);
      const halfAngle = (c.coneAngle || 60) * Math.PI / 360;
  ...
  _updateAndDrawChainLightnings(dt) {
    ...
    for (let i = this.chainLightningEffects.length - 1; i >= 0; i--) {
      ...
      cl.age += dt || 1/60;
      if (cl.age >= cl.maxAge) { this.chainLightningEffects.splice(i, 1); continue; }
