...
class TownScreen {
  constructor({ audioManager, gameManager, eventBus, dataManager, companionSystem, estateSystem, affectionSystem, farmingSystem, disasterSystem, sandboxSystem, startGame, getPendingDisaster, clearPendingDisaster }) {
    ...
    // D1: Location Manager
    this.locationManager = new LocationManager(this.gameManager);
    this.locationManager.onNavigate = (loc) => this._onLocationNavigate(loc);
    
    // D3: Shop System
    this.shopSystem = new ShopSystem(this.gameManager, this.eventBus);
  ...
  _updateDisplay() {
    ...
    if (pendingDisaster) {
      this._showDisasterNotification(pendingDisaster);
